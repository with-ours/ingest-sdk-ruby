...
module OursPrivacy
  module Resources
    class Track
      ...
      #
      # @return [OursPrivacy::Models::TrackEventResponse]
      #
      # @see OursPrivacy::Models::TrackEventParams
      def event(params)
        parsed, options = OursPrivacy::TrackEventParams.dump_request(params)
        path = @client.base_url_overridden? ? "track" : "https://api.oursprivacy.com/api/v1/track"
        @client.request(
          method: :post,
          path: path,
          body: parsed,
          model: OursPrivacy::Models::TrackEventResponse,
          options: options
        )
      end

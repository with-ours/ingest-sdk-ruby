...
class OursprivacyIngest::Test::SingletonClient < OursprivacyIngest::Client
  include Singleton

  TEST_API_BASE_URL = ENV.fetch("TEST_API_BASE_URL", "http://localhost:4010")

  def initialize
    super(base_url: OursprivacyIngest::Test::SingletonClient::TEST_API_BASE_URL)
  end
end
...
class OursprivacyIngest::Test::ResourceTest < Minitest::Test
  def async?
  ...
  def before_all
    super
    @ours_privacy = OursprivacyIngest::Test::SingletonClient.instance
  end

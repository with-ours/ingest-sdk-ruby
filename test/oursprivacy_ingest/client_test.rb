...
class OursprivacyIngestTest < Minitest::Test
  ...
  def test_client_default_request_default_retry_attempts
    stub_request(:post, "http://localhost/track").to_return_json(status: 500, body: {})

    ours_privacy = OursprivacyIngest::Client.new(base_url: "http://localhost")

    assert_raises(OursprivacyIngest::Errors::InternalServerError) do
  ...
  def test_client_given_request_default_retry_attempts
    ...
    ours_privacy = OursprivacyIngest::Client.new(base_url: "http://localhost", max_retries: 3)
  ...
  def test_client_default_request_given_retry_attempts
    ...
    assert_raises(OursprivacyIngest::Errors::InternalServerError) do
      ours_privacy.track.event(token: "x", event: "x", request_options: {max_retries: 3})
    end
  ...
  def test_client_given_request_given_retry_attempts
    ...
    assert_raises(OursprivacyIngest::Errors::InternalServerError) do
      ours_privacy.track.event(token: "x", event: "x", request_options: {max_retries: 4})
    end
  ...
  def test_client_retry_after_seconds
    stub_request(:post, "http://localhost/track").to_return_json(
      status: 500,
      headers: {"retry-after" => "1.3"},
      body: {}
    )

    ours_privacy = OursprivacyIngest::Client.new(base_url: "http://localhost", max_retries: 1)
  ...
  def test_client_redirect_307
    ...
    assert_raises(OursprivacyIngest::Errors::APIConnectionError) do
      ours_privacy.track.event(token: "x", event: "x", request_options: {extra_headers: {}})
    end

    recorded, = WebMock::RequestRegistry.instance.requested_signatures.hash.first

    assert_requested(:any, "http://localhost/redirected", times: OursprivacyIngest::Client::MAX_REDIRECTS) do
  ...
  def test_client_redirect_303
    ...
    assert_requested(:get, "http://localhost/redirected", times: OursprivacyIngest::Client::MAX_REDIRECTS) do
  ...
  def test_client_redirect_auth_strip_cross_origin
    ...
    assert_requested(
      ...
      times: OursprivacyIngest::Client::MAX_REDIRECTS
    ) do
      headers = _1.headers.keys.map(&:downcase)
  ...
  def test_default_headers
    stub_request(:post, "http://localhost/track").to_return_json(status: 200, body: {})
